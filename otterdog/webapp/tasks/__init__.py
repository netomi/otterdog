...
from otterdog.providers.github.rest.auth.token import TokenAuthStrategy
...
def _create_rest_api_for_app() -> RestApi:
    ...
    return RestApi(AppAuthStrategy(github_app_id, github_app_private_key))
...
def get_rest_api_for_installation(installation_id: int) -> RestApi:
    ...
    rest_api = RestApi(TokenAuthStrategy(token))
