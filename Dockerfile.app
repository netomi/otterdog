...
COPY otterdog.* ./
COPY pyproject.toml poetry.lock README.md gunicorn-cfg.py ./
...
COPY --from=builder-python3 /app/gunicorn-cfg.py /app/gunicorn-cfg.py
...
# gunicorn
CMD ["/app/.venv/bin/gunicorn", "--config", "gunicorn-cfg.py", "otterdog.app:app"]